...
class PairDataset(object):
    ...
    def build_vocab(self, embed_file: str = None) -> Vocab:
        ...
        filtered = word_counts.most_common(config.max_vocab_size)
        list_to_add = [word for word, count in filtered]
        vocab.add_words(list_to_add)
...
class SampleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            'len_OOV': len(oov),
            'y': [self.vocab.SOS] +
                 abstract2ids(self.trg_sents[index], self.vocab, oov) +
                 [self.vocab.EOS],
